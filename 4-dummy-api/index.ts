...
type SSN = `${number}-${number}-${number}` // Номер социального обеспечения
type EIN = `${number}-${number}` // Идентификационный номер работодателя
...
type Address = {
    address: string,
    city?: string, // Не обязательное поле
...
function isObjectSimilarToType<T>(value: unknown): value is Record<keyof T, unknown> {
...
function isCoordinates(value: unknown): value is Coordinates {
    ...
    // TODO: часто в коде попадаются конструкции с проверкой типа и сообщением об ошибке
    //  в дальнейшем можно вынести в отдельную функцию
...
function isBank(value: unknown): value is Bank {
    ...
    if (!isCardNumber(value.cardNumber)) {
        console.error('Ошибка: поле \'cardNumber\' не соответствует типу CardNumber.');
        return false;
    }
...
function isAddress(value: unknown): value is Address {
    ...
    if (!isCoordinates(value.coordinates)) {
        console.error('Ошибка: поле \'coordinates\' не соответствует типу Coordinates.');
        return false;
    }
...
function isUsersResponse(value: unknown): value is UsersResponse {
    if (!isObjectSimilarToType<UsersResponse>(value)) {
        console.error("Ошибка: значение не является объектом, подобным UsersResponse.");
        return false;
    }
    ...
    for (let i = 0; i < value.users.length; i++) {
        ...
        if (!isUser(currentUser)) {
            ...
            console.error(`Ошибка: элемент с индексом ${i} в поле 'users' не соответствует типу User.`);
